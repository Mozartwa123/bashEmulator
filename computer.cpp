...
/*
    Niestety za późno zorientowałem się, jak TRUDNE jest rozdzielanie plików w c++ (nagłówki bez
    namespace std; implementacje metod w pliku cpp). Zmiana idei kosztowała by mnie zaczynanie od
    początku (nawarstwienie problemów). Z tego powodu (prawie) wszystko jest w jednym pliku. 
    Przepraszam zatrudności.
*/
...
class MyDirectory : public MemObject, enable_shared_from_this<MyDirectory>{
    public:
        ...
        MyDirectory(string name, bool isScriptObject, shared_ptr<MyDirectory> parent) {
            this->objname = name;
        ...
        static shared_ptr<MyDirectory> create(string name, bool isScriptObject, shared_ptr<MyDirectory> parent) {
        ...
        string givePath(){
            if(this->path == ""){
                ...
                (this->parentDir->giveObjName() == "/" ? "/" + this->giveObjName() : 
...
class File : public MemObject {
    public:
        ...
        string givePath(){
            if(this->path == ""){
                this->path = (this->giveObjName() == "/" ? "/" : 
                (this->localization->giveObjName() == "/" ? "/" + this->giveObjName() : 
            this->localization->givePath() + "/" + this->giveObjName()));
            }
            return this->path;
        }
    };
...
class Computer {
    public:
        ...
        Computer(string username, string cmpname) {
            shared_ptr<MyDirectory> rootDirectory = MyDirectory::create("/", false, nullptr);
            this->rootDirectory = rootDirectory;
            allDirectories.push_back(rootDirectory);
            allDirectories.push_back(MyDirectory::create("etc", false, rootDirectory));
            shared_ptr<MyDirectory> homeDirectory = MyDirectory::create("home", false, rootDirectory);
            //this -> homeDirectory = homeDirectory;
            allDirectories.push_back(homeDirectory);
            //allDirectories.push_back(MyDirectory::create("home", false, rootDirectory));
            ...
            allDirectories.push_back(MyDirectory::create("lib64", false, rootDirectory));
            ...
            /* TODO: USER DIRECTORY!!!!*/
            this->currentDirectory = MyDirectory::create(username, false, homeDirectory);
            this -> userDirectory = this -> currentDirectory;
            allDirectories.push_back(MyDirectory::create("documents", false, this->userDirectory));
            allDirectories.push_back(MyDirectory::create(".config", false, this->userDirectory));
            allDirectories.push_back(MyDirectory::create(".local", false, this->userDirectory));
...
class Command{
    public:
      ...
      function<string(vector<string> flags, vector<string> arguments, shared_ptr<Computer>, shared_ptr<Command>)> execute;
      
      friend class Console;
      Command(string name,
        string desc,
        map<string, SafetyStatus> flags,
        function<string(vector<string>, vector<string>, shared_ptr<Computer> , shared_ptr<Command>)> func)
    : commandName(name), description(desc), availableFlags(flags), execute(func) {}
};

string cd(vector<string> flags, vector<string> arguments, shared_ptr<Computer> computer, shared_ptr<Command> command){
    ...
    if(arguments[0] == "."){
        ...
        return computer->currentDirectory->giveObjName(); //przechodzimy do tego samego katalogu
    }
    else if(arguments[0] == ".."){
        ...
        computer->currentDirectory = computer->currentDirectory->parentDir; //przechodzimy do rodzica
        ...
        if(dir == nullptr){
            fprintf(stderr, "Given directory %s does not exist\n", (computer->currentDirectory->giveObjName() + (computer->currentDirectory->giveObjName() == "/" ? "" : "/") + arguments[0]).c_str());
            return "";
        }
    }
    if(dir == nullptr){
...
string clear(vector<string> flags, vector<string> arguments, shared_ptr<Computer> computer, shared_ptr<Command> command){
    cout << "\033[2J\033[1;1H";
    return "\033[2J\033[1;1H";
}

string fastfetch(vector<string> flags, vector<string> arguments, shared_ptr<Computer> computer, shared_ptr<Command> command) {
    ...
    vector<string> infoLines = {
        ...
        "Kernel: none",
        "Packages: 1 [snake]", //skoro jest pacman, to dlaczego nie ma być snake'a?
        "Terminal: tty42",
...
string pwd(vector<string> flags, vector<string> arguments, shared_ptr<Computer> computer, shared_ptr<Command> command){
...
string listRec(shared_ptr<MyDirectory> listedDir, vector<string> flags, ostringstream& output){
    output << RESET;
    if(!listedDir->childrenDir.empty()){
        output<<endl;
    }
    for(shared_ptr<File> fil : listedDir->childrenFil){
        if(fil->giveObjName()[0]=='.' && find(flags.begin(), flags.end(), "a") == flags.end()){
            continue;
        }
    ...
    for(shared_ptr<MyDirectory> dir : listedDir->childrenDir){
        if(dir->giveObjName()[0]=='.' && find(flags.begin(), flags.end(), "a") == flags.end()){
            continue;
        }
...
string ls(vector<string> flags, vector<string> arguments, shared_ptr<Computer> computer, shared_ptr<Command> command){
...
class Interpreter{
    ...
    Interpreter(shared_ptr<Computer> computer){
        availableCommands["cd"] = make_shared<Command>("cd", "entering directories", map<string, SafetyStatus>{}, cd);
        availableCommands["pwd"] = make_shared<Command>("pwd", "showing current localization", map<string, SafetyStatus>{}, pwd);
        availableCommands["fastfetch"] = make_shared<Command>("fastfetch", "showing technical info", map<string, SafetyStatus>{}, fastfetch);
        availableCommands["clear"] = make_shared<Command>("clear", "clearing console", map<string, SafetyStatus>{}, clear);
        availableCommands["ls"] = make_shared<Command>("ls", "listing directory content", map<string, SafetyStatus>{}, ls);
    ...
    string evalCmd(string cmdname, vector<string>& args, vector<string>& flags){
        ...
        return cmd->execute(flags, args, this->computer, cmd);
    }
        ...
        switch (cmdLine->type) {
            ...
            case NodeType::And:
                if(auto* ands = std::get_if<andStruct>(&cmdLine->value)){
                    return evalFromAst(ands->left, pipeStack) + evalFromAst(ands->right, pipeStack);
                }
            case NodeType::Pipe: {
    ...
    string eval(string input){
        return evalFromAst(syntaxConverting(input), {});
    }
};
...
class Console{
    ...
    Console(){
        ...
        //do skanowania zrobię chyba flagę uruchomienia, ale jeśli skuszę się na GUI to będzie problem...
        string cmpName;
        string usrName;
    ...
    private:
    void bashSession(){
        cout<< RED << this->computer->username<< MAGENTA <<"@"<< GREEN <<computer->cmpname + " "<< YELLOW << 
        ((computer->currentDirectory == computer->userDirectory) ? "~": computer->currentDirectory->giveObjName()) 
        << RESET << " $ ";
        string n;
        getline(cin, n);
        if(n == "end"){
